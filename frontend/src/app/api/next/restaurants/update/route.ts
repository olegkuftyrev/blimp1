...
export async function PUT(req: NextRequest) {
  try {
    ...
    const endpoint = `restaurants/${storeId}`
    ...
    if (!res.ok) {
      ...
      console.error(`Backend PUT /restaurants/${storeId} failed: ${res.status} - ${errorText}`)
    ...
    console.error('API route PUT /api/next/restaurants/update error:', error)
